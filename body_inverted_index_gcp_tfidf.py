...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"postings_gcp_bwtfidf/{file_name}")
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        self.n = 0
        self.wikiid_title = defaultdict(list) # {wiki id: title,...}
        self.term_idf = defaultdict(list) # {term: idf,...}
        self.len_doc = defaultdict(list) # {doc: len doc,...}
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                ...
                for i in range(self.df[w]):
                    ...
                    # doc_id, tf = struct.unpack('If', b[i * TUPLE_SIZE:(i + 1) * TUPLE_SIZE])
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        blob_posting_locs = bucket.blob(f"postings_gcp_bwtfidf/{bucket_id}_posting_locs.pickle")
